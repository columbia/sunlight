class Analytics::Filter::PvalueHack
  def self.filter(expt_id, output_id, training_set, ordered_inputs, options={})
    ...
    # compute the observed alpha on the training set
    # for the whole combination
    training_set_alpha = training_set.select do |profile_id|
      mappings_full_hash[profile_id] &&
      (mappings_full_hash[profile_id].keys & ordered_inputs).count > 0
    end.count / training_set.count.to_f
